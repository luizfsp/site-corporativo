...
function goTranslate(id_elemento) {
    ...
    if (arquivoAtual == "index.html" || arquivoAtual == "index.aspx" || arquivoAtual == "" || arquivoAtual == "home") {
        ...
        if (location.href.indexOf(".aspx") >= 0) {
            url = location.href.replace(location.href.substring(location.href.lastIndexOf("/"), location.href.length), "") + "/index.aspx";
        }
        else if (location.href.indexOf(".html") >= 0) {
            url = location.href.replace(location.href.substring(location.href.lastIndexOf("/"), location.href.length), "") + "/index.html";
        }
        else {
            /*window.location.href = location.href.replace(location.href.substring(location.href.lastIndexOf("/"), location.href.length), "") + "/";*/
            url = location.origin + "/";
        }
        if (window.location.href.indexOf('c2tiapps') > -1 && window.location.href.indexOf('emp') > -1) {
            var emp = window.location.href.substring(window.location.href.indexOf('emp'));
            if (url.indexOf('?') > -1) url += ('&' + emp);
            else url += ('?' + emp);
        }
        console.log(url);
